...
import { usePoolAnalysis, useRefreshPool, usePoolHistory, usePoolTicks, useRawPool, useILData } from '../hooks/usePoolData.ts'
...
export default function PoolDetail({ chain, address, onBack }: Props) {
  ...
  const { data: ilData }                   = useILData(chain, address)
  ...
  // Prezzo corrente token0/token1 dal giorno più recente (history30 è ordinato desc)
      ...
      {data && (
        <>
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="flex items-center gap-3 mt-1">
                ...
                <span className="text-xs font-mono text-gray-500">{data.poolAddress}</span>
              </div>
            </div>
          ...
          <div className="flex gap-1 border-b border-gray-800">
            {(['overview', 'charts', 'il'] as Tab[]).map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
          ...
          {tab === 'overview' && (
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              ...
              <ScoreMatrix parameters={data.parameters} />
            </div>
          )}
          ...
          {tab === 'il' && (
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              {ilData ? (
                <ILSimulator
                  points={ilData.points}
                  currentFeeAPR={ilData.currentFeeAPR}
